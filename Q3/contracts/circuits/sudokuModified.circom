...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range[i][j] = RangeProof(32); //[assignment] change assert() to use your created RangeProof instead
            range[i][j].range[0] <==0; //lower bound
            range[i][j].range[1] <==9; // higher bound
            range[i][j].in <== puzzle[i][j]; // set input = puzzle square selected
            range[i][j].out === 1; //in range check
